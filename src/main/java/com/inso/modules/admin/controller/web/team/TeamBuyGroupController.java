...
public class TeamBuyGroupController {
...
//    private TransferOrderService mTransferOrderService;
    ...
    @RequiresPermissions("root_web_team_buying_group_list")
    @RequestMapping("root_web_team_buying_group")
    public String toList(Model model, HttpServletRequest request)
    {
        ...
        return "admin/web/team_buy/team_buy_group_list";
    }

    @RequiresPermissions("root_web_team_buying_group_list")
    @RequestMapping("getWebTeamBuyingGroupList")
    @ResponseBody
    public String getDataList()
    {
        ...
        RowPager<TeamBuyGroupInfo> rowPager = mTeamBuyGroupService.queryScrollPage(pageVo, agentid, staffid, userid, businessType, status);

        template.setData(rowPager);
        return template.toJSONString();
    }

    @RequiresPermissions("root_web_team_buying_group_edit")
    @RequestMapping("toWebTeamBuyingGroupEditPage")
    public String toEdigPage(Model model, HttpServletRequest request)
    {
        long id = WebRequest.getLong("id");
        if(id > 0)
        {
            TeamBuyGroupInfo entity = mTeamBuyGroupService.findById(false, id);
            model.addAttribute("entity", entity);
        }

        ICurrencyType.addModel(model);
        TeamBusinessType.addModel(model);
        return "admin/web/team_buy/team_buy_group_edit";
    }

    @RequiresPermissions("root_web_team_buying_group_edit")
    @RequestMapping("updateWebTeamBuyingGroupInfo")
    @ResponseBody
...
//        ICurrencyType currency = ICurrencyType.getType(WebRequest.getString("currencyType"));
        long hasInviteCount = WebRequest.getLong("hasInviteCount");
        long hasCompleteCount = WebRequest.getLong("hasCompleteCount");
...
//                    txStatus = OrderTxStatus.REALIZED;
                    hasInviteCount = entity.getNeedInviteCount();
                }

                mTeamBuyGroupService.updateInfo(entity, hasInviteCount, status);
            }
            else
            {
