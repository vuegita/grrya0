...
public class AndarBaharConfigController {

    @Autowired
    private ConfigService mConfigService;

    @RequiresPermissions("root_game_andar_bahar_config_list")
    @RequestMapping("root_game_andar_bahar_config")
    public String toBasicPlatformConfig(Model model)
    {
        List<ConfigKey> configList = mConfigService.findByList(false, "game_andar_bahar");
        ...
        return "admin/game/game_andar_bahar_config";
    }

    @RequiresPermissions("root_game_andar_bahar_config_edit")
    @RequestMapping("updateGameAndarBaharConfig")
    @ResponseBody
    public String updateGameLotteryConfig()
    {
        ...
        for (String key : keys)
        {
            ...
            {
                mConfigService.updateValue("game_andar_bahar:" + key, value);
            }
        }

        // 更新缓存
        mConfigService.findByList(true, "game_andar_bahar");
    ...
    private void updateNumberConfigToDB(String key, long minValue, long maxValue)
    {
        ...
        {
            mConfigService.updateValue("game_andar_bahar:" + key, value + StringUtils.getEmpty());
        }
    }
