...
import com.inso.modules.game.lottery_game_impl.mines.MinelHandleManager;
...
public class MinesMessageImpl implements MyMessageSupport {

    private static Log LOG = LogFactory.getLog(MinesMessageImpl.class);

    private LotteryV2Api mLotteryV2Api;
    private MinelHandleManager mFootballHandleManager;
    public MinesMessageImpl()
    {
        this.mLotteryV2Api = SpringContextUtils.getBean(LotteryV2Api.class);
        this.mFootballHandleManager = SpringContextUtils.getBean(MinelHandleManager.class);
    }

    @Override
    public MyGroupType getGroupType() {
        return MyGroupType.GAME_MINES;
    }
...
//            rs = mLotteryV2Api.getLotteryStatus_internal(null, jsonMsg);
        ...
        {
            rs = mLotteryV2Api.getLatestBetRecord_internal(jsonMsg, MyGroupType.GAME_MINES.getKey());
        }
        else if(eventType == MyEventType.GAME_STATUS_GET_USER_CURRENT_BET_RECORD)
        {
            rs = mLotteryV2Api.getUserBetRecord_internal(jsonMsg, MyGroupType.GAME_MINES.getKey());
        }
