...
@Repository
public class TeamOrderDaoMysql extends DaoSupport implements TeamOrderDao {
    ...
    /**
     ...
     order_activityid             int(11) UNSIGNED NOT NULL ,
     order_receiv_rid             int(11) UNSIGNED NOT NULL comment '参加活动的领取ID' ,
     ...
     */
    private static final String TABLE = "inso_web_team_buying_order";

    @Override
    public void addOrder(String orderno, String outTradeNo, UserAttr userAttr, TeamBuyGroupInfo groupInfo, long recordid,
                         BigDecimal amount, ICurrencyType currency, OrderTxStatus txStatus,
                         TeamBusinessType orderType, JSONObject remark)
    {
        ...
        keyvalue.put("order_business_type", orderType.getKey());

        keyvalue.put("order_group_id", groupInfo.getId());
        keyvalue.put("order_record_id", recordid);

        if(groupInfo.getUserid() == userAttr.getUserid())
        {
            keyvalue.put("order_index_id", groupInfo.getHasInviteCount() + 1);
        }
        else
        {
            keyvalue.put("order_index_id", 0);
        }
    ...
    public TeamOrderInfo findById(String orderno)
    {
        ...
        return mSlaveJdbcService.queryForObject(sql, TeamOrderInfo.class, orderno);
    }
    ...
    public RowPager<TeamOrderInfo> queryScrollPage(PageVo pageVo, String sysOrderno, long agentid, long staffid, long userid, OrderTxStatus status)
    {
        ...
        List<TeamOrderInfo> list = mSlaveJdbcService.queryForList(select.toString(), TeamOrderInfo.class, values.toArray());
        RowPager<TeamOrderInfo> rowPage = new RowPager<>(total, list);
        return rowPage;
    }
