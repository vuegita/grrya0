...
@Repository
public class ABPeriodDaoMysql extends DaoSupport implements ABPeriodDao {

    private static final String TABLE = "inso_game_andar_bahar_period";
...
//        keyvalue.put("period_status", "pending");
        ...
        keyvalue.put("period_open_card_num", openCardNum);
    ...
    public ABPeriodInfo findByTime(String time, ABType type)
    {
        ...
        return mSlaveJdbcService.queryForObject(sql, ABPeriodInfo.class, time, type.getKey());
    }

    public ABPeriodInfo findByIssue(String issue)
    {
        String sql = "select * from " + TABLE + " where period_issue = ?";
        return mSlaveJdbcService.queryForObject(sql, ABPeriodInfo.class, issue);
    }

    public List<ABPeriodInfo> queryByTime(ABType type, String beginTime, String endTime, int limit)
    {
        ...
        return mSlaveJdbcService.queryForList(sql, ABPeriodInfo.class, beginTime, endTime, type.getKey());
    }
    ...
    public void queryAll(ABType type, String startTimeString, String endTimeString, Callback<ABPeriodInfo> callback)
    {
        if(type == null)
        {
            ...
            mSlaveJdbcService.queryAll(callback, sql, ABPeriodInfo.class, startTimeString, endTimeString);
        }

        else
        {
            String sql = "select * from " + TABLE + " where period_starttime between ? and ? and period_type = ?";
            mSlaveJdbcService.queryAll(callback, sql, ABPeriodInfo.class, startTimeString, endTimeString, type.getKey());
        }
    ...
    public RowPager<ABPeriodInfo> queryScrollPage(PageVo pageVo, String issue, ABType type, GamePeriodStatus status)
    {
        ...
        String countsql = "select count(1) from inso_game_andar_bahar_period " + whereSQL;
        ...
        StringBuilder select = new StringBuilder("select * from inso_game_andar_bahar_period ");
        select.append(whereSQL);
        select.append(" order by period_starttime asc ");
        select.append(" limit ").append(pageVo.getOffset()).append(",").append(pageVo.getLimit());
        List<ABPeriodInfo> list = mSlaveJdbcService.queryForList(select.toString(), ABPeriodInfo.class, values.toArray());
        RowPager<ABPeriodInfo> rowPage = new RowPager<>(total, list);
        return rowPage;
    }
}
