package com.inso.modules.game.andar_bahar.controller;
...
import com.inso.modules.game.andar_bahar.logical.ABBetTaskManager;
...
public class Api {
    ...
    @Autowired
    private ABBetTaskManager mBetConcurrent;

    @Autowired
    private ABOrderService mOrderService;

    /**
     * @api {post} /game/abApi/getStatus
    ...
    public String getStatus()
    {
        ...
        ABType abType = ABType.PRIMARY;

        ABRunningStatus runningStatus = ABRunningStatus.tryLoadCache(abType);
        ...
        ABPeriodStatus periodStatus = ABPeriodStatus.tryLoadCache(false, abType, runningStatus.getCurrentIssue());
        ...
        if(periodStatus.getOpenResult() != null && !CollectionUtils.isEmpty(periodStatus.getmCardOriginNumberList()))
        {
            dataMpas.put("targetCardList", periodStatus.getmCardOriginNumberList());
        }
...
//    @MyLoginRequired
    public String getCurrentStatus()
    {
        ...
        ABType abType = ABType.PRIMARY;
        ABPeriodStatus periodStatus = ABPeriodStatus.tryLoadCache(false, abType, issue);
        if(periodStatus == null)
        {
            apiJsonTemplate.setJsonResult(SystemErrorResult.ERR_SYS_BUSY);
            return apiJsonTemplate.toJSONString();
...
//        dataMpas.put("status", runningStatus.verify());
    ...
    /**
     * @api {post} /game/abApi/submitOrder
     * @apiDescription  提交订单
     * @apiName submitOrder
     * @apiGroup Game-ab
     * @apiVersion 1.0.0
...
//    @MyIPRateLimit(maxCount = 20, expires = 50)
    ...
    public String submitOrder()
    {
        ...
        ABBetItemType betItem = ABBetItemType.getType(betItemString);
        if(betItem == null)
        {
        ...
        //
        ABPeriodStatus status = ABPeriodStatus.tryLoadCache(false, abType, runningStatus.getCurrentIssue());
        if(status == null)
        {
        ...
        //
        boolean rs = mBetConcurrent.addItemToQueue(abType, runningStatus.getCurrentIssue(), userInfo, basicAmount, betCount, betItem);
        if(!rs)
        {
            apiJsonTemplate.setJsonResult(SystemErrorResult.ERR_SYS_BUSY);
        }
    ...
    /**
     * @api {post} /game/abApi/getLatestBetRecord
...
//        LotteryABType lotteryType = LotteryABType.getType(lotteryTypeStr);
        ABType abType = ABType.PRIMARY;
...
//        boolean refreshRobotData = false; // 刷新频率
        ...
        List<JSONObject> list = MyLotteryBetRecordCache.getInstance().getAllRecordListFromCache(false, abType, status.getIssue());
        if(!CollectionUtils.isEmpty(list))
        {
            for(JSONObject jsonObject : list)
            {
                ...
                // 获取开奖结果
                String openResult = ABOpenResultHelper.getOpenResult(abType, issue);
                if(!StringUtils.isEmpty(openResult))
                {
                    long openResultNumber = StringUtils.asInt(openResult);

                    // 开奖结果
                    jsonObject.put(MyLotteryBetRecordCache.KEY_OPEN_RESULT, ABOpenResultHelper.getOpenResult(abType, issue));
                    // 是否中奖
    ...
    /**
     * @api {post} /game/abApi/getUserCurrentBetRecord
     * @apiDescription  获取最新投注记录-当前运行的期号
...
//        LotteryABType lotteryType = LotteryABType.getType(lotteryTypeStr);
        ...
        List<JSONObject> list = MyLotteryBetRecordCache.getInstance().getUserRecordListFromCache(false, abType, username);
        if(!CollectionUtils.isEmpty(list))
        {
            for(JSONObject jsonObject : list)
            {
    ...
    /**
     * @api {post} /game/abApi/getUserHistoryBetRecord
    ...
    public String getUserHistoryBetRecord()
    {
        ...
        if("waiting".equalsIgnoreCase(type))
        {
            ...
            loadAllCurrentUserBetRecordForWaiting(rsList, userInfo.getName(), abType);
        ...
        else
        {
            ...
            List<ABOrderInfo> rsList = mOrderService.queryListByUserid(timeString, userInfo.getId(), offset);

            if(!CollectionUtils.isEmpty(rsList))
            {
                List list = Lists.newArrayList();

                for(ABOrderInfo model : rsList)
                {
                    ABType tmpType = ABType.getType(model.getType());
                    ...
                    {
                        maps.put(MyLotteryBetRecordCache.KEY_OPEN_RESULT, ABOpenResultHelper.getOpenResult(tmpType, model.getIssue()));
                    }
                    else
                    {
    ...
    /**
     * @api {post} /game/abApi/getLatestPeriodList
    ...
    public String getLatestPeriodList()
    {
        ...
        List list = ABLatestPeriodCache.getCache(abType, offset);
        apiJsonTemplate.setData(list);
        return apiJsonTemplate.toJSONString();
    }
     ...
     * @param lotteryType
     */
    private void loadAllCurrentUserBetRecordForWaiting(List<JSONObject> resultList, String username, ABType lotteryType)
    {
        ...
        ABPeriodStatus status = ABPeriodStatus.tryLoadCache(false, lotteryType, runningStatus.getCurrentIssue());
        if(status == null)
        {
            return;
        }
