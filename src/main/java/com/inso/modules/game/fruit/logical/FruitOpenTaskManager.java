 ...
 */
@Component
public class FruitOpenTaskManager {


    private static Log LOG = LogFactory.getLog(FruitOpenTaskManager.class);
    ...
    private static FruitBetItemType[] mBetItemTypeArray = FruitBetItemType.values();

    @Autowired
    private FruitPeriodService mPeriodService;

    @Autowired
    private FruitOrderService mAOrderService;
    ...
    private FruitBetItemType mLastOpenResult = null;

    private boolean debug = false;

    public void handleOpenResultForAllOrder(String issue, FruitBetItemType openResult)
    {
        // 系统维护不执行开奖订单信息
        ...
        // 重新支付
        mAOrderService.queryAllByIssue(issue, new Callback<FruitOrderInfo>() {
            @Override
            public void execute(FruitOrderInfo orderInfo) {

                //清除下注前100条缓存记录
                String cachekey = GameCacheKeyHelper.queryOrderLatestPage_100(GameCategory.FRUIT, null, orderInfo.getUserid());
...
//                    }
                    ...
                    FruitBetItemType betItemType = FruitBetItemType.getType(orderInfo.getBetItem());
                    // 中奖充值
                    BigDecimal winMoney = FruitHelper.calcWinMoney(orderInfo.getBasicAmount(), orderInfo.getBetCount(), openResult, betItemType);
                    if(winMoney != null && winMoney.compareTo(BigDecimal.ZERO) > 0)
                    {
                        ErrorResult errorResult = mPayApiMgr.doBusinessRecharge(accountType, currencyType, BusinessType.GAME_FRUIT, orderInfo.getNo(), userInfo, winMoney, null);
    ...
    public FruitBetItemType getOpenResult(FruitPeriodInfo model, float platformRate, int smartNum)
    {
    ...
    private void updateOpenMode(FruitPeriodInfo model, GameOpenMode dbMode, GameOpenMode currentOpenMode)
    {
    ...
    private FruitBetItemType getOpenResultByRandom()
    {
        FruitBetItemType openResult = FruitOpenResultHelper.randomOpenItem();
        ...
        try {
            ...
            if(mLastOpenResult != null && openResult == mLastOpenResult)
            {
                ...
                {
                    openResult = FruitOpenResultHelper.randomOpenItem();
                }
            }
    ...
    private FruitBetItemType getByCalcAllOrder(FruitPeriodInfo model, float platformRate)
    {
        //
        LinkedHashMap<String, Object> maps = getCalcMaps();
        FruitBetItemType[] betItemTypes = FruitBetItemType.values();
        for (FruitBetItemType type : betItemTypes)
        {
        ...
        mAOrderService.queryAllByIssue(model.getIssue(), new Callback<FruitOrderInfo>() {
            public void execute(FruitOrderInfo orderInfo) {
                FruitBetItemType betItemType = FruitBetItemType.getType(orderInfo.getBetItem());

                if(betItemType == FruitBetItemType.DW){
                    doCalcAmount(maps, orderInfo.getBasicAmount(), orderInfo.getBetCount(),FruitBetItemType.DW, betItemType,isIncreseTotalBetAmount);
                    doCalcAmount(maps, orderInfo.getBasicAmount(), orderInfo.getBetCount(), FruitBetItemType.XDW, betItemType,isIncreTotalBetAmount);

                }else if(betItemType == FruitBetItemType.SX){
                    doCalcAmount(maps, orderInfo.getBasicAmount(), orderInfo.getBetCount(),FruitBetItemType.SX, betItemType,isIncreseTotalBetAmount);
                    doCalcAmount(maps, orderInfo.getBasicAmount(), orderInfo.getBetCount(),FruitBetItemType.XSX, betItemType,isIncreTotalBetAmount);
                }else if(betItemType == FruitBetItemType.XG){
                    doCalcAmount(maps, orderInfo.getBasicAmount(), orderInfo.getBetCount(),  FruitBetItemType.XG,betItemType,isIncreseTotalBetAmount);
                    doCalcAmount(maps, orderInfo.getBasicAmount(), orderInfo.getBetCount(), FruitBetItemType.XXG, betItemType,isIncreTotalBetAmount);
                }else if(betItemType == FruitBetItemType.QQ){
                    doCalcAmount(maps, orderInfo.getBasicAmount(), orderInfo.getBetCount(), FruitBetItemType.QQ,betItemType,isIncreseTotalBetAmount);
                    doCalcAmount(maps, orderInfo.getBasicAmount(), orderInfo.getBetCount(), FruitBetItemType.XQQ,betItemType,isIncreTotalBetAmount);
                }else if(betItemType == FruitBetItemType.HZ){
                    doCalcAmount(maps, orderInfo.getBasicAmount(), orderInfo.getBetCount(), FruitBetItemType.HZ,betItemType,isIncreseTotalBetAmount);
                    doCalcAmount(maps, orderInfo.getBasicAmount(), orderInfo.getBetCount(), FruitBetItemType.XHZ, betItemType,isIncreTotalBetAmount);
                }else if(betItemType == FruitBetItemType.PG){
                    doCalcAmount(maps, orderInfo.getBasicAmount(), orderInfo.getBetCount(), FruitBetItemType.PG,betItemType,isIncreseTotalBetAmount);
                    doCalcAmount(maps, orderInfo.getBasicAmount(), orderInfo.getBetCount(), FruitBetItemType.XPG,betItemType,isIncreTotalBetAmount);
                }else if(betItemType == FruitBetItemType.JZ){
                    doCalcAmount(maps, orderInfo.getBasicAmount(), orderInfo.getBetCount(), FruitBetItemType.JZ,betItemType,isIncreseTotalBetAmount);
                    doCalcAmount(maps, orderInfo.getBasicAmount(), orderInfo.getBetCount(), FruitBetItemType.XJZ,betItemType,isIncreTotalBetAmount);
                }else if(betItemType == FruitBetItemType.NM){
                    doCalcAmount(maps, orderInfo.getBasicAmount(), orderInfo.getBetCount(), FruitBetItemType.NM,betItemType,isIncreseTotalBetAmount);
                    doCalcAmount(maps, orderInfo.getBasicAmount(), orderInfo.getBetCount(), FruitBetItemType.XNM,betItemType,isIncreTotalBetAmount);
                }
        ...
        //
        FruitBetItemType rsOpenResult = doCalcOpenResult(maps, platformRate);
        return rsOpenResult;
    }

    private FruitBetItemType doCalcOpenResult(LinkedHashMap<String, Object> maps, float platformRate)
    {
    ...
    private FruitBetItemType getOpenResultByItem(CalcItem firstItem, CalcItem secondItem)
    {
        // 进入这里已经是属于比例开奖，但是如果计算结果是开Tie，则 1/10 杀
        if( firstItem.openResult == FruitBetItemType.DW) {
            int number = RandomUtils.nextInt(50);
            if(number== 8){
                return FruitBetItemType.DW;
            }
            if(secondItem != null)
            {
    ...
    private void doCalcAmount(LinkedHashMap<String, Object> maps, BigDecimal basicAmount, long betCount, FruitBetItemType openResult , FruitBetItemType betItem ,boolean isIncreTotalBetAmount)
    {
//        System.out.println("betItem " + betItem.getKey() + ", openResult = " + openResult.getKey());
        String key = openResult.getKey();
        ...
        BigDecimal winmoney = FruitHelper.calcWinMoney(basicAmount, betCount, openResult, betItem);
    ...
    private class CalcTask {
        ...
        public void incre(FruitOrderInfo orderInfo)
        {
            totalBetCount ++;
        }
    ...
    private  LinkedHashMap<String, Object> getCalcMaps()
    {
        ...
        for(FruitBetItemType tmp : mBetItemTypeArray)
        {
            CalcItem item = new CalcItem();
    ...
    public static void main(String[] args) {
        FruitOpenTaskManager mgr = new FruitOpenTaskManager();
...
//            int betItem = RandomUtils.nextInt(3);
            ...
            mgr.doCalcAmount(maps, basicAmount, betCount, FruitOpenResultHelper.randomOpenItem(),FruitOpenResultHelper.randomOpenItem(),true);
        }

        mgr.debug = true;
        FruitBetItemType openResult = mgr.doCalcOpenResult(maps, 0.85f);
