 ...
 */
@Component
public class FruitBetTaskManager {

    private static Log LOG = LogFactory.getLog(FruitBetTaskManager.class);
    ...
    public boolean addItemToQueue(FruitType type, String issue, UserInfo userInfo, long amountValue, long betCount, FruitBetItemType betItem)
    {
        if(isFull())
        {
            return false;
    ...
    public ErrorResult doCreateOrder(FruitType type, String issue, UserInfo userInfo, long basicAmountValue, long betCount, FruitBetItemType betItem)
    {
        try {
            synchronized (userInfo.getName())
            {
                FruitPeriodStatus runningStatus = FruitPeriodStatus.loadCache(false, type, issue);
                ...
                String orderno = FruitHelper.nextOrderId(type);
                ...
                BigDecimal feemoney = FruitHelper.getTotalFeemoney(basicAmount, betCount);
                ...
                mFruitOrderService.addOrder(orderno, issue, type, userInfo, userAttr, betItem, basicAmount, betCount, totalBetAmount, feemoney, null);

                // pay
                ErrorResult result = mPayApiMgr.doBusinessDeduct(accountType, currencyType, BusinessType.GAME_FRUIT, orderno, userInfo, totalBetAmount, feemoney, null);
                if(result == SystemErrorResult.SUCCESS)
                {
                    mFruitOrderService.updateTxStatus(orderno, OrderTxStatus.WAITING);
    ...
    private class BetItemTask implements Runnable{
        ...
        private FruitBetItemType betItem;
        ...
        public FruitBetItemType getBetItem() {
            return betItem;
        }

        public void setBetItem(FruitBetItemType betItem) {
