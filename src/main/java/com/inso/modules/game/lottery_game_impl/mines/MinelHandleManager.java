package com.inso.modules.game.lottery_game_impl.mines;
...
import com.inso.modules.game.lottery_game_impl.mines.helper.MinesHelper;
...
@Component
public class MinelHandleManager implements IMessageAsyncNotify {

    private static final String ROOT_CACHE = MinelHandleManager.class.getName();
    ...
    private static Log LOG = LogFactory.getLog(MinelHandleManager.class);


    private static final String SALT = "fsad)(*&)3423lsdf'l;'l;/768fa23df";
    ...
    // 用户下注选择几个地雷
    public static String KEY_BET_ITEM_MINES_NUM = "betItem";
    ...
    private GameChildType childType = MineType.Mines;
    ...
    public void handleAction(JSONObject jsonObject, MyEventType eventType, Session session)
    {
        ...
        if(StringUtils.isEmpty(accessToken))
        {
            ...
            apiJsonTemplate.setEvent(MyGroupType.GAME_MINES.getKey(), eventType.getKey());
        ...
        synchronized (username)
        {
            if(eventType == MyEventType.GAME_SUBMIT_ORDER)
            {
                ...
                if(statusData != null)
                {
                    ...
                    long cacheBeginTime = statusData.getLongValue(FootballHandleManager.KEY_BEGIN_TIME);
                    apiJsonTemplate.setData(statusData);

                    OrderTxStatus txStatus = OrderTxStatus.getType(statusData.getString(FootballHandleManager.KEY_TX_STATUS));
                    // 过期1小时结算
                    ...
                    {
                        String orderno = statusData.getString(FootballHandleManager.KEY_ORDER_NO);
                        String betItem = statusData.getString(FootballHandleManager.KEY_BET_INDEX);
            ...
            {
                String rs = handleToBet(jsonObject, username, null);
                WebSocketServer.sendMessage(rs, session);
            }
    ...
    public String handleToBet(JSONObject jsonObject, String username, ApiJsonTemplate apiJsonTemplate)
    {
        if(apiJsonTemplate == null)
        {
            apiJsonTemplate = new ApiJsonTemplate();
        }
        apiJsonTemplate.setEvent(MyGroupType.GAME_MINES.getKey(), MyEventType.GAME_BET_ORDER_STEP.getKey());
...
//            statusData = createAndSaveOrderStatus(null, 0, "order1", OrderTxStatus.NEW, username, new BigDecimal(5));
        ...
        // 用户点击
        int userChooseIndex = jsonObject.getIntValue("userChooseIndex");
        if(userChooseIndex < 0 || userChooseIndex >= 25)
        {
            apiJsonTemplate.setJsonResult(SystemErrorResult.ERR_PARAMS);
            return apiJsonTemplate.toJSONString();
        }
        ...
        //
        int userBetMinesNum = statusData.getIntValue(KEY_BET_ITEM_MINES_NUM);

        // verify-betIndex
        int dbIndex = statusData.getIntValue(FootballHandleManager.KEY_BET_INDEX);
        if(dbIndex >= 25)
        {
        ...
        // verify-sign
        String sign = jsonObject.getString(FootballHandleManager.KEY_SIGN);
        ...
        // 会员中奖:   随机数(100 - 当前地雷顺序) > (后台配置杀率 * 100 )  , 备注：挖雷越多难度越大
        // 验证规则1: 游戏本身-规则
        int gameCount = 25 - dbIndex;
        boolean verifyProfite = gameCount > userBetMinesNum;
...
//            double normalRate = Math.pow((25.0f - userBetMinesNum) / 25.0f,  index);
//            double normalRate = (25.0f - userBetMinesNum - dbIndex) / (25.0f - dbIndex);
            // 从1开始
            double normalRate = MinesHelper.calcProbability(userBetMinesNum, index);
            double dbCount = refreshAndGetProfitRate() * normalRate;
//            double basicRate = refreshAndGetProfitRate()* ( 25  - userBetMinesNum)/25.0f;
//            double dbCount = Math.pow(basicRate, dbIndex);

            verifyProfite = sysRandomCount < dbCount; // 0.7 => 0.7 * (2 / 25) * 100
            LOG.info("Mines: sysRandomCount = " + sysRandomCount + ", normalRate = " + normalRate + ", dbCfgCount = " + dbCount + ", indexCount = " + index + ", orderno = " + orderno);
        }
...
//        txStatus = OrderTxStatus.WAITING;
        ...
        if((dbIndex + userBetMinesNum) >= 25 && txStatus == OrderTxStatus.WAITING)
        {
            txStatus = OrderTxStatus.REALIZED;
        }

        // save
        createAndSaveOrderStatus(statusData, index, orderno, txStatus, username, null, userChooseIndex, null);
...
//        System.out.println("index = " + index);
    ...
    {
        ApiJsonTemplate apiJsonTemplate = new ApiJsonTemplate();
        apiJsonTemplate.setEvent(MyGroupType.GAME_MINES.getKey(), MyEventType.GAME_CASHOUT_ORDER.getKey());
        ...
        try {
            ...
            // save
            txStatus = OrderTxStatus.REALIZED;
            createAndSaveOrderStatus(statusData, dbIndex, orderno, txStatus, username, null, -1, null);
    ...
    public void onBetFinish(String sessionid, ErrorResult result, String orderno, UserInfo userInfo, BigDecimal betAmount, String[] betItemArr) {
        ...
        ApiJsonTemplate apiJsonTemplate = new ApiJsonTemplate();
        apiJsonTemplate.setEvent(MyGroupType.GAME_MINES.getKey(), MyEventType.GAME_SUBMIT_ORDER.getKey());
        ...
        String minesNum = betItemArr[0];

        // 处理状态
        JSONObject statusData = createAndSaveOrderStatus(null, 0, orderno, OrderTxStatus.NEW, userInfo.getName(), betAmount, -1, minesNum);
...
//        String rs = handleToBet(true, statusData, userInfo.getName(), apiJsonTemplate);
    ...
    public JSONObject createAndSaveOrderStatus(JSONObject statusData, int index, String orderno, OrderTxStatus txStatus, String username, BigDecimal betAmount, int chooseNum, String minesNum)
    {
        if(statusData == null)
        {
        ...
        {
            // 会员点击历史记录
            String historyBetArr = StringUtils.getNotEmpty(statusData.getString(FootballHandleManager.KEY_BET_INDEX_ORDER));
            if(index == 1)
            {
                historyBetArr = chooseNum + StringUtils.getEmpty();
            }
            else
            {
                historyBetArr += StringUtils.COMMA + chooseNum;
            }
            statusData.put(FootballHandleManager.KEY_BET_INDEX_ORDER, historyBetArr);
        }

        // 用户
        if(!StringUtils.isEmpty(minesNum))
        {
            statusData.put(KEY_BET_ITEM_MINES_NUM, minesNum);
        }

        statusData.put(FootballHandleManager.KEY_BET_INDEX, index);

        statusData.put(FootballHandleManager.KEY_ORDER_NO, orderno);
        if(betAmount != null)
        {
            statusData.put(FootballHandleManager.KEY_BET_AMOUNT, betAmount);
            statusData.put(FootballHandleManager.KEY_BEGIN_TIME, System.currentTimeMillis());
        }
        statusData.put(FootballHandleManager.KEY_TX_STATUS, txStatus.getKey());
        statusData.put(FootballHandleManager.KEY_SIGN, sign);
    ...
    private float refreshAndGetProfitRate()
    {
        ...
        float value = mConfigService.getFloat(false, "game_mines:open_rate");
        if(value >= 0)
        {
    ...
    public static void main(String[] args) {
        ...
        for(int i = 0; i < 10; i ++)
        {
            double normalRate = (25.0f - userBetMinesNum- dbIndex ) / 25.0f;
            double dbCount = 100 * normalRate;
            System.out.println("sysRandomCount = " + sysRandomCount + ", basic = " + normalRate + ", dbCount " + dbCount + ", clickCount = " + dbIndex);
            dbIndex ++;
        }
