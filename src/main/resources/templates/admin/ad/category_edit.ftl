    ...
    <div class="ibox float-e-margins">
        ...
        <div class="ibox-content">
            ...
            <form id="form" class="form-horizontal" autocomplete="off">
                ...
                <div class="form-group">
                    <label class="control-label col-sm-2"><span class="text-danger">*&nbsp;</span>最低价格:</label>
                    <div class="col-sm-5">
                        <input class="form-control" name="minPrice" id="minPrice"
                               onkeyup="this.value=this.value.replace(/[^\d]/g,'');"
                               value="<#if entity?exists> ${entity.minPrice} </#if>" autocomplete="off" required maxlength="100"/>
                    </div>
                </div>
                <div class="hr-line-dashed"></div>

                <div class="form-group">
                    <label class="control-label col-sm-2"><span class="text-danger">*&nbsp;</span>最高价格:</label>
                    <div class="col-sm-5">
                        <input class="form-control" name="maxPrice" id="maxPrice"
                               onkeyup="this.value=this.value.replace(/[^\d]/g,'');"
                               value="<#if entity?exists> ${entity.maxPrice} </#if>" autocomplete="off" required maxlength="100"/>
                    </div>
                </div>
                <div class="hr-line-dashed"></div>

                <div class="form-group">
                    <label class="control-label col-sm-2">返佣利率:</label>
                    <div class="col-sm-5">
                        <input class="form-control" name="returnRate" id="returnRate" autocomplete="off"
                               required type="text" onkeyup="this.value=this.value.replace(/[^\d\.]/g,'');"
                               placeholder="0 < X < 1"
                               value="<#if entity?exists> ${entity.returnRate?string('#.####')  !} </#if> " />
                    </div>
                    <div class="col-sm-5" style="color: red">1. 日收益率范围0~1, 例：1%的话填写0.01</div>
                    <div class="col-sm-5" style="color: red">2. 针对Shop</div>
                </div>
    ...
    $(function () {
        ...
        $("#submitBtn").click(function () {
            ...
            $.ajax({
                type: "post",
                async: false,
                url: "/alibaba888/Liv2sky3soLa93vEr62/editAdCategoryInfo",
                data: {
                    id:id,
                    name: name,
                    minPrice:minPrice,
