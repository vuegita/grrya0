...
<#--                </div>-->
                ...
                <div class="pull-left" style="margin-right: 10px;">
                    <input type="text" name="categoryid" id="categoryid" class="form-control input-outline"  placeholder="分类ID" style="width:100px;">
                </div>
...
<#--                </div>-->
            ...
            <table id="bootstrapTable">
                ...
                <tr>
                    ...
                    <th data-field="categoryid" data-width="5%" data-align="center"
                        class="title">分类ID
                    </th>
...
<script type="text/javascript">
    ...
    function actionFormatter(value, row, index) {
        var result = "";
        result += '<a href="javascript:;" class="table-btn editMateriel" data-no="' + row.id + '" title="编辑">编辑</a>';

        if(row.eventType == "shop")
        {
            result += '<a href="javascript:;" class="table-btn addCommodity" data-no="' + row.id + '" title="关联商家">关联商家</a>';
            result += '<a href="javascript:;" class="table-btn editMaterielDetail" data-no="' + row.id + '" title="编辑内容">编辑内容</a>';
        }
        return result;
    }
    ...
    $(function () {
        ...
        options.url = '/alibaba888/Liv2sky3soLa93vEr62/getAdMaterielList';
        ...
        $(document).on('click', '.table-btn.editMaterielDetail', function () {
            var id = $(this).attr('data-no');
            var url = "/alibaba888/Liv2sky3soLa93vEr62/toEditAdMaterielDetailPage?id=" + id;
            openNewWindow(url);
        });

        $(document).on('click', '.table-btn.addCommodity', function () {
            var id = $(this).attr('data-no');
            var url = "/alibaba888/Liv2sky3soLa93vEr62/toEditAdMallCommodityPage?materialid=" + id;
            openNewWindow(url);
        });
