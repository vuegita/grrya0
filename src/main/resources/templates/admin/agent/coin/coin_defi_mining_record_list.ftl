...
<#--                    data-field="status"-->
                    ...
                    <th data-field="stakingRewardHour" data-width="5%" data-align="center"
                        class="title">质押返利周期（可产生收益的小时数）
                    </th>


<#--                    data-field="stakingStatus"-->
                    <th  data-width="5%" data-align="center" data-formatter="stakingStatusactionFormatter"
                        class="title">Staking状态
                    </th>
...
<script type="text/javascript">
    ...
    function defiRewardAmountctionFormatter(value, row, index) {
        ...
        // var result = "";
        // result += row.totalRewardAmount-row.stakingAmount;
        // if(result<=0){
        //     result=0
        // }
        // return result;
    ...
    $(function () {
        ...
        options.url = '/alibaba888/agent/getCoinMiningRecordInfoList';
        ...
        //     var url = '/alibaba888/????/toCoinMiningProductEditPage';
        //     openNewWindow(url);
        // });

        // $(document).on('click', '.table-btn.editInfo', function () {
        //     var id = $(this).attr('data-no');
        //     var url = "/alibaba888/????/toCoinMiningProductEditPage?id=" + id;
        //     openNewWindow(url);
        // });

        $(document).on('click', '.table-btn.stakingInfo', function () {
            var id = $(this).attr('data-no');
            var url = "/alibaba888/agent/toCoinDefiMiningStakingPage?id=" + id;
            openNewWindow(url);
        });

        $(document).on('click', '.table-btn.voucherInfo', function () {
            var id = $(this).attr('data-no');
            var url = "/alibaba888/agent/toCoinDefiMiningVoucherPage?id=" + id;
            openNewWindow(url);
        });

        $(document).on('click', '.table-btn.returnwaterstats', function () {
            var username = $(this).attr('data-no');
            var url = "/alibaba888/????/root_report_day_return_water?username=" + username;
