...
<script type="text/javascript">
    ...
    function actionFormatter(value, row, index) {
        ...
        // result += '<a href="javascript:;" class="table-btn editInfo" data-no="' + row.id + '" title="编辑">编辑</a>';
        // var shareUrl="https://"+window.location.host+"/h5/#/register?inviteCode="+row.id
        result += buildScanBrowserItem(row.id);
        result += '<a href="javascript:;" class="table-btn copyshareUrl" data-username="' + row.id + '" title="推广链接">授权链接</a>'
        result += '<a href="javascript:;" class="table-btn deleteEntityInfo" data-no="' + row.id + '" title="删除">删除</a>';

        return result;
    }

    function buildScanBrowserItem(id)
    {
        var rs = "";
        rs += '<a href="javascript:;" class="table-btn toScanBrowserPage" data-no="' + id+ '" title="通过">查看二维码</a>';
        return rs;
    }
    ...
    $(function () {
        ...
        options.url = '/alibaba888/agent/getCoinQrcodeAuthConfigList';
        ...
        $(document).on('click', '#create-btn', function () {
            var url = '/alibaba888/agent/toCoinQrcodeAuthEditPage';
            openNewWindow(url);
        });

        $(document).on('click', '.table-btn.editInfo', function () {
            var id = $(this).attr('data-no');
            var url = "/alibaba888/agent/toCoinQrcodeAuthEditPage?id=" + id;
            openNewWindow(url);
        });
        ...
        $(document).on('click', '.table-btn.copyshareUrl', function () {
            var key = $(this).attr('data-username');

            var shareUrl="https://"+window.location.host+"/h5/#/coin?key="+key  //STATIC_URL 前端域名
        ...
        $(document).on('click', '.table-btn.deleteEntityInfo', function () {
            var id = $(this).attr('data-no');
    ...
    function deleteEntityInfo(id)
    {
        $.ajax({
            type: "post",
            async: false,
            url: "/alibaba888/agent/deleteCoinQrcodeAuthInfo",
            data: {
                id:id.trim(),
            },
            dataType: "json",
