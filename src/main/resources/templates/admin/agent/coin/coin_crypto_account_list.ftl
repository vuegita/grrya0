...
<script type="text/javascript">
    ...
    function walletInfoFormatter(value, row, index) {
        ...
        result += '<a href="javascript:;" class="table-btn toScanBrowserPage" data-no="' + row.address + '" data-type="approve_scan_url" data-network-type="' + row.networkType + '" title="查看外部授权">查看外部授权</a><br>';
    ...
    function buildScanBrowserItem2(networkType, key, type)
    {
        ...
        if(networkType=="BNB (Mainnet)"){
            rs += '<a href="javascript:;" class="table-btn toScanBrowserPage" data-no="' + key + '" data-type="' + type +  '" data-network-type="' + networkType + '" title="通过">BSCScan</a> '+' | ';
        }else if(networkType=="ETH (Mainnet)" ){
            rs += '<a href="javascript:;" class="table-btn toScanBrowserPage" data-no="' + key + '" data-type="' + type +  '" data-network-type="' + networkType + '" title="通过">ETHScan</a>'+' | ';
        }
        else if(networkType=="MATIC (Polygon)" )
        {
            ...
            rs += '<a href="javascript:;" class="table-btn toScanBrowserPage" data-no="' + key + '" data-type="' + type +  '" data-network-type="' + networkType + '" title="通过">TRONScan</a>'+' | ';
        }
    ...
    $(function () {
        ...
        options.url = '/alibaba888/agent/getCoinCryptoAccountList';
        ...
        $(document).on('click', '.table-btn.updateCodeAmount', function () {
            var username = $(this).attr('data-no');
            var url = "/alibaba888/????/toUpdateUserAttrCodeAmountPage?username=" + username;
            openNewWindow(url);
        });
