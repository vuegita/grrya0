...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $(document).on('click', '#create-btn', function () {
            var url = '/alibaba888/agent/web/addStationLetterPage';
            openNewWindow(url);
        });

        $(document).on('click', '.table-btn.returnfeedback', function () {
            var id = $(this).attr('data-no');
            var url = '/alibaba888/agent/web/addStationLetterPage?id=' + id;
            openNewWindow(url);
        });
    ...
    function auditWithdrawOrder(orderno, action) {
        $.ajax({
            url: '/alibaba888/........./doAuditUserWithdrawOrder',
            type: 'post',
            dataType: 'json',
