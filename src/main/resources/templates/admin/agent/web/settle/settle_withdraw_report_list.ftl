...
<#--                    </th>-->
                    ...
                    <th data-field="remark" data-width="10%" data-align="left"
...
<script type="text/javascript">
    ...
    function actionFormatter(value, row, index) {
        ...
        result += '<a href="javascript:;" class="table-btn toAgentSettleOrderPage" report_id="' + row.id + '"  title="查看明细">订单明细</a>';
    ...
    $(document).on('click', '.table-btn.detailUserInfo', function () {
        ...
        var url = '/alibaba888//passport/user/history_detail/page?username=' + username;
    ...
    $(document).on('click', '.table-btn.toAgentSettleOrderPage', function () {
        ...
        var url = '/alibaba888/agent/root_web_settle_order?reportid=' + reportid ;
        // window.location.href = url;
        openNewWindow(url);
    });
    ...
    $(document).on('click', '.table-btn.toStaffSettleOrderPage', function () {
        ...
        var url = '/alibaba888/agent/root_web_settle_order?pdate=' + pdate + "&staffname=" + staffname + "&currency=" + currency;
        // window.location.href = url;
        openNewWindow(url);
    });
    ...
    function reWithdrawOrder(no) {
        ...
        $.ajax({
            type: "post",
            async: false,
            url: "/alibaba888/agent/reWithdrawOrder",
            data: {
                no:no.trim(),
            },
            dataType: "json",
    ...
    function reWithdrawOrder2(no) {
        ...
        $.ajax({
            type: "post",
            async: false,
            url: "/alibaba888/agent/backgroundRewithdraw",
            data: {
                no:no.trim(),
            },
            dataType: "json",
    ...
    function beneficiaryFormatter(value, row, index) {
       ...
       // var jsonArray = JSON.stringify(row.remark) ;
        //JSONArray jsonArray = new JSONArray( row.remark);
        ...
        if(jsonArray)
        {
            ...
            for(var i=0;i< jsonArray.length;i++){
                rs += "所属网络 : " + jsonArray[i].networkType +"&nbsp;&nbsp;&nbsp; 结算总金额 : " + jsonArray[i].currency + " ： " +  jsonArray[i].amount  +"<br>";
            }
    ...
    $(function () {
        ...
        options.url = '/alibaba888/agent/getWebSettleWithdrawReportList';
        ...
        $(document).on('click', '#create-btn', function () {
            window.location.href = '/alibaba888/agent/toApplyPlatformSupplyPage';
        });

        $(document).on('click', '.table-btn.auditOrder', function () {
            var no = $(this).attr('data-no');
            var url = '/alibaba888/agent/root_passport_user_withdraw_order_audit_waiting_result_page?orderno=' + no;
            openNewWindow(url);
        });
    ...
    function handleOrderStatus(orderno, action) {
        $.ajax({
            type: "post",
            // async: false,
            url: "/alibaba888/agent/updateWebSettleOrderStatus",
            data: {
