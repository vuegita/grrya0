...
<div class="wrapper wrapper-content animated fadeInRight">
    <div class="ibox float-e-margins">
        <div class="ibox-title">
            <h5>编辑客服</h5>
            <div style="color: red">1.请输入正确后台员工的名称否则会添加失败</div>
            <div style="color: red">2.每个员工可以添加不同客服类型各一条，不能重复添加已经有的客服类型</div>
            <div style="color: red">3.客服类型选chatwoot时请先查看文档 <a href="javascript:;" class="table-btn redPUrl"  style="text-decoration: underline;font-weight: bold">下载文档</a></div>
            <div style="color: red">4.客服类型选chatwoot时客服号填 websiteToken 的值,客服链接填 0.</div>
            <div style="color: red">3.客服类型选tawk时请先查看文档 <a href="javascript:;" class="table-btn redPUrl2"  style="text-decoration: underline;font-weight: bold">下载文档</a></div>
            <div style="color: red">5.客服类型选tawk时客服号填 0 ,客服链接填复制内容中 s1.src 的值.</div>
            <div style="color: red">6.推荐客服页面颜色编码 #2962ef（可忽略）</div>
        ...
        <div class="ibox-content">
            <form id="form" class="form-horizontal" autocomplete="off">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-3">
                        <input class="form-control" id="staffname" name="staffname" autocomplete="off" required maxlength="50"  <#if staffKefu?exists>readonly</#if> value="<#if staffKefu??>${staffKefu.staffname}</#if>" />
                    </div>
...
<script type="text/javascript">
    $(function () {
        ...
        $(document).on('click', '.table-btn.redPUrl', function () {
            var url = "https://"+window.location.host+"/mining/text/chatwoot.docx"
            //var url = "https://todefi.finance/mining/text/chatwoot.docx"
            window.open(url)
        });


        $(document).on('click', '.table-btn.redPUrl2', function () {
            var url = "https://"+window.location.host+"/mining/text/tawk.docx"
            window.open(url)
        });
...
//isEmpty(staffname) ||
            if( isEmpty(title) || isEmpty(describe) || isEmpty(whatsapp) || isEmpty(telegram) || isEmpty(status))
            {
                $.global.openErrorMsg('* 号必填参数不能为空');
                return false;
            }
...
//isEmpty(staffname) ||
        ...
        $.ajax({
            url: '/alibaba888/agent/web/editWebStaffKefu',
            type: 'post',
            dataType: 'json',
            data:{
                id:id,
