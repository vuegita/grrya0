...
<#--                    </th>-->
                    ...
                    <th data-field="id" data-formatter="userInfoFormatter" data-width="10%" data-align="left">用户信息 & 订单信息</th>
...
<script type="text/javascript">
    ...
    function userInfoFormatter(value, row, index) {
        ...
        result += "业务类型: " + row.businessType + " <br>";
        result += "系统订单: " + row.no + " <br>";
        if(row.outTradeNo)
        {
            result += "业务订单: " + row.outTradeNo + " <br>";
        }
        result += "创建时间: " + row.createtime + " <br>";

        var remarkMsg = columnFormatterForOrderRemarkMsg(value, row, index);
        if(remarkMsg && remarkMsg.length > 0)
        {
            result += "备注信息: " + remarkMsg + " <br>";
        }
        return result;
    }
