...
<script type="text/javascript">
    ...
    function detailInfoFormatter(value, row, index) {
        ...
        result += "拼团分组 : " + row.groupId + "<br>";
        if(row.recordId == 0)
        {
            result += "拼团记录 : " + row.recordId + " (创建者)<br>";
        }
        else
        {
            result += "拼团记录 : " + row.recordId + " (参与者)<br>";
        }
        ...
        try {
            ...
            result += "赠送比例 : " + remark.presentRate + "<br>";
        } catch (e) {
        }
    ...
    $(function () {
        ...
        options.url = '/alibaba888/Liv2sky3soLa93vEr62/getWebTeamBuyOrderInfoList';
    ...
    function handleOrder(action, orderno)
    {
        $('#myModalDel').hide();
        $.ajax({
            url: '/alibaba888/Liv2sky3soLa93vEr62/updateWebTeamBuyOrderInfo',
            type: 'post',
            data: {
                action:action,
                orderno:orderno,
