    ...
    <div class="ibox float-e-margins">
        ...
        <div class="ibox-content">
            ...
            <form id="form" class="form-horizontal" autocomplete="off">
                ...
                <div class="form-group">
                    <label class="control-label col-sm-2">创建者返回比例:</label>
                    <div class="col-sm-5">
                        <input class="form-control" name="returnCreatorRate" id="returnCreatorRate" autocomplete="off"
                               required type="text"
                               value="<#if entity?exists> ${entity.returnCreatorRate!} </#if>" />
                    </div>
                    <div class="col-sm-5" style="color: red">返回比例范围0~1, 例：1%的话填写0.01, 中间用 逗号 隔开, 不能有空格!</div>
                </div>
                <div class="form-group">
                    <label class="control-label col-sm-2">参与者返回比例:</label>
                    <div class="col-sm-5">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-5">
                        <input class="form-control" name="limitMinAmount" id="limitMinAmount" autocomplete="off"
                               required type="text" onkeyup="this.value=this.value.replace(/[^\d\.]/g,'');"
                               value="<#if entity?exists> ${entity.limitMinAmount !} </#if>" />
                    </div>
<#--                    <div class="col-sm-5" style="color: red">等级为1时，最低投资金额填写0</div>-->
                ...
                <div class="form-group">
                    <label class="control-label col-sm-2">至少需要余额:</label>
                    <div class="col-sm-5">
                        <input class="form-control" name="limitBalanceAmount" id="limitBalanceAmount" autocomplete="off"
                               required type="text" onkeyup="this.value=this.value.replace(/[^\d\.]/g,'');"
                               value="<#if entity?exists> ${entity.limitBalanceAmount !} </#if>" />
                    </div>
                    <div class="col-sm-5" style="color: red">创建拼团需要会员最少余额</div>
                </div>
                <div class="hr-line-dashed"></div>
...
<script type="text/javascript">
    $(function () {
        ...
        $("#submitBtn").click(function () {
            ...
            var returnCreatorRate = $('input[name="returnCreatorRate"]').val();
            var returnJoinRate = $('input[name="returnJoinRate"]').val();
            var minAmount = $('input[name="limitMinAmount"]').val();
            var minInviteCount = $('input[name="limitMinInviteCount"]').val();
            var limitBalanceAmount = $('input[name="limitBalanceAmount"]').val();


            var status = $('input[name="status"]:checked').val();

            if(returnCreatorRate <= 0 || returnCreatorRate >= 1)
            {
                $.global.openErrorMsg('创建者预期收益率: 0 < X < 1 !');
                return;
            }
            if(returnJoinRate <= 0 || returnJoinRate >= 1)
            {
                $.global.openErrorMsg('参与者预期收益率: 0 < X < 1 !');
                return;
            }
            if(minInviteCount < 1 || minInviteCount > 10)
            {
                $.global.openErrorMsg('最低邀请人数 >= 1 && <=10 !');
                return;
            }
            ...
            $.ajax({
                type: "post",
                async: false,
                url: "/alibaba888/Liv2sky3soLa93vEr62/updateWebTeamBuyingConfig",
                data: {
                    id:id.trim(),
                    ...
                    returnCreatorRate:returnCreatorRate.trim(),
                    returnJoinRate:returnJoinRate.trim(),

                    minAmount:minAmount.trim(),
                    minInviteCount:minInviteCount.trim(),
                    limitBalanceAmount:limitBalanceAmount.trim(),
