...
<#--                <div class="pull-left" style="margin-right: 10px;">-->
<#--                    <input type="text" name="staffname" id="staffname" class="form-control input-outline"  placeholder="请输入员工名"  value="" style="width:150px;">-->
<#--                </div>-->

<#--                <div class="pull-left" style="margin-right: 10px;">-->
<#--                    <input type="text" name="useranme" id="useranme" class="form-control input-outline"  placeholder="请输入用户名"  value="" style="width:150px;">-->
<#--                </div>-->
...
<#--                    <th data-field="businessType" data-width="5%" data-align="center"-->
<#--                        class="title">所属业务-->
<#--                    </th>-->
...
<#--                    </th>-->
                    ...
                    <th data-field="limitMinInviteCount" data-width="5%" data-align="center"
                        class="title">最低邀请人数
                    </th>

                    <th data-field="limitMinInvesAmount" data-width="5%" data-align="center"
                        class="title">最低投资金额
                    </th>

<#--                    <th data-field="basicPresentAmount" data-width="5%" data-align="center"-->
<#--                        class="title">基础赠送金额-->
<#--                    </th>-->

<#--                    <th data-field="extraPresentTier" data-width="5%" data-align="center"-->
<#--                        class="title">额外赠送比例-->
<#--                    </th>-->
                    ...
                    <th data-field="username" data-width="5%" data-align="left"
                        data-formatter="finishStatsFormatter"
                        class="title">活动结果
                    </th>
...
<script type="text/javascript">
    ...
    function detailInfoFormatter(value, row, index) {
        ...
        result += "所 属 ID: " + row.id + " <br>";
        // result += "用 户 名: " + row.username + " <br>";
        // result += "所属代理: " + row.agentname + " <br>";
        // result += "挖矿员工: " + row.staffname + " <br>";
        result += "所属业务: " + row.businessType + " <br>";
        result += "创建时间: " + row.createtime + " <br>";
        result += "开始时间: " + row.begintime + " <br>";
        result += "结束时间: " + row.endtime + " <br>";

        result += "基础赠送金额: " + row.basicPresentAmount + " <br>";
        result += "额外赠送层级: " + row.extraPresentTier + " <br>";
        return result;
    }

    function finishStatsFormatter(value, row, index) {
        var result = "";

        result += "邀请总人数: " + row.finishInviteCount + " <br>";
        result += "投资总金额: " + row.finishInvesAmount + " <br>";
        result += "赠送总金额: " + row.finishPresentAmount + " <br>";

        if(row.finishInvesAmount > 0)
        {
            result += "活动总收益: " + (row.finishInvesAmount - row.finishPresentAmount) + " <br>";
        }
    ...
    function actionFormatter(value, row, index) {
        ...
        if(row.status == "realized" && isSuperAdmin)
        {
            result += '<a href="javascript:;" class="table-btn resettleActivity" data-no="' + row.id + '" title="重新结算">重新结算</a>';
        }

        if(row.status == "new")
        {
            result += '<a href="javascript:;" class="table-btn deleteInfo" data-no="' + row.id + '" title="删除">删除</a>';
        }
    ...
    $(function () {
        ...
        options.url = '/alibaba888/Liv2sky3soLa93vEr62/getWebActivityConfigList';
        ...
        $(document).on('click', '#create-btn', function () {
            var url = '/alibaba888/Liv2sky3soLa93vEr62/toWebActivityConfigEditPage';
            openNewWindow(url);
        });

        $(document).on('click', '.table-btn.editInfo', function () {
            var id = $(this).attr('data-no');
            var url = "/alibaba888/Liv2sky3soLa93vEr62/toWebActivityConfigEditPage?id=" + id;
            openNewWindow(url);
        });
        ...
        $(document).on('click', '.table-btn.deleteInfo', function () {
            ...
            $('#myModalDel #type').attr('action', 'deleteInfo');
    ...
    function deleteInfo(id)
    {
        ...
        console.log("id = " + id);
        $.ajax({
            url: '/alibaba888/Liv2sky3soLa93vEr62/deleteWebActivityInfo',
            type: 'post',
            data: {id:id},
            dataType: 'json',
