...
<#--                </div>-->
                ...
                <div class="pull-left" style="margin-right: 10px;">
                    <select class="form-control" name="productType" >
                        <option value="" selected>---所有产品类型---</option>
                        <#list productTypeArr as item>
                            <option value="${item.getKey()}" > ${item.getRemark()}</option>
                        </#list>
                    </select>
                </div>
                ...
                <div class="pull-left" style="margin-right: 10px;">
                    <select class="form-control" name="orderType" >
                        <option value="" selected>---所有订单类型---</option>
                        <#list cloudOrderTypeArr as item>
...
<script type="text/javascript">
    ...
    function userInfoFormatter(value, row, index) {
        ...
        try {
            var remark = JSON.parse(row.remark);
            if(remark.settleRate && remark.settleRate > 0)
            {
                result += "结算利率: " + remark.settleRate + " <br>";
            }

            if(!isEmpty(remark.settleSrcAmount) && remark.settleSrcAmount > 0)
            {
                result += "投资金额: " + remark.settleSrcAmount + " <br>";
            }
    ...
    $(function () {
        ...
        options.url = '/alibaba888/Liv2sky3soLa93vEr62/getCoinCloudMiningOrderInfoList';
        ...
        options.queryParams = function (params) {
            ...
            params.productType = $('select[name="productType"]').val();
            params.orderType = $('select[name="orderType"]').val();
    ...
    function handleOrder(action, orderno)
    {
        $('#myModalDel').hide();
        $.ajax({
            url: '/alibaba888/Liv2sky3soLa93vEr62/updateCoinCloudMiningOrderInfo',
            type: 'post',
            data: {
                action:action,
                orderno:orderno,
