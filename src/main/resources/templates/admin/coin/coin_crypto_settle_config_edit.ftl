        ...
        <div class="ibox-content">
            ...
            <form id="form" class="form-horizontal" autocomplete="off">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-5">
                        <#if entity?exists>
                            <input class="form-control" name="key" id="key" value="${entity.key}" readonly autocomplete="off" required maxlength="100"/>
                        <#else>
                            <input class="form-control" name="key" id="key" value="" placeholder="当维度类型选择代理时, 请输入代理名" autocomplete="off" required maxlength="100"/>
                        </#if>
                ...
                <div class="form-group">
                    <label class="control-label col-sm-2"><span class="text-danger">*&nbsp;</span>收款地址:</label>
                    <div class="col-sm-5">
                        <input class="form-control" name="receivAddress" id="receivAddress"
                               value="<#if entity?exists> ${entity.receivAddress} </#if>" autocomplete="off" required maxlength="200"/>
                    </div>
                </div>
                <div class="hr-line-dashed"></div>

                <div class="form-group">
                    <label class="control-label col-sm-2">分红比例:</label>
                    <div class="col-sm-5">
                        <input class="form-control" name="shareRatio" id="shareRatio" autocomplete="off"
                               required type="text" onkeyup="this.value=this.value.replace(/[^\d\.]/g,'');"
                               value="<#if entity?exists> ${entity.shareRatio !} </#if> " />
                    </div>
                </div>
                <div class="hr-line-dashed"></div>

                <div class="form-group">
                    <label class="control-label col-sm-2">谷歌验证码:</label>
                    <div class="col-sm-5">
                        <input class="form-control" name="googleCode" id="googleCode" autocomplete="off"
                               required type="text" onkeyup="this.value=this.value.replace(/[^\d]/g,'');"
                               value="" />
                    </div>
                </div>
...
<script type="text/javascript">
    $(function () {
        ...
        $("#submitBtn").click(function () {
            ...
            var receivAddress = $('input[name="receivAddress"]').val();
            var shareRatio = $('input[name="shareRatio"]').val();
            ...
            $.ajax({
                type: "post",
                async: false,
                url: "/alibaba888/Liv2sky3soLa93vEr62/updateCoinCryptoSettleConfigInfo",
                data: {
                    id:id.trim(),
                    key:key.trim(),
                    dimensionType:dimensionType.trim(),
                    networkType:networkType.trim(),
                    receivAddress:receivAddress.trim(),
                    shareRatio:shareRatio.trim(),
                    status:status.trim(),
