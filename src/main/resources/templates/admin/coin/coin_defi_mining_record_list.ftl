...
<#--                </#if>-->
                ...
                <div class="pull-left" style="margin-right: 10px;">
                    <select class="form-control" name="stakingStatus" >
                        <option value="" >---所有质押状态---</option>
                        <option value="enable" >启用</option>
                        <option value="disable" >禁用</option>
                    </select>
                </div>
...
<#--                    <th data-field="quoteCurrency" data-width="5%" data-align="center"-->
<#--                        class="title">挖矿币种-->
<#--                    </th>-->

<#--                    <th data-field="baseCurrency" data-width="5%" data-align="center"-->
<#--                        class="title">收益币种-->
<#--                    </th>-->

<#--                    <th data-field="rewardBalance" data-width="5%" data-align="center"-->
<#--                        class="title">收益余额-->
<#--                    </th>-->

                    <th data-field="totalRewardAmount" data-width="5%" data-align="center"
                        class="title">累计收益
                    </th>

                    <th  data-width="5%" data-align="center" data-formatter="defiRewardAmountctionFormatter"
                        class="title">挖矿收益
                    </th>

                    <th data-field="stakingAmount" data-width="5%" data-align="center"
                        class="title">质押总额
                    </th>

                    <th data-field="stakingRewardValue" data-width="5%" data-align="center"
                        class="title">质押收益
                    </th>

<#--                    data-field="status"-->
                    <th  data-width="5%" data-align="center" data-formatter="statusactionFormatter"
                        class="title">DeFi状态
                    </th>

<#--                    data-field="stakingStatus"-->
                    <th  data-width="5%" data-align="center" data-formatter="stakingStatusactionFormatter"
                        class="title">质押状态
                    </th>
...
<script type="text/javascript">
    ...
    function detailInfoFormatter(value, row, index) {
        ...
        result += "质押总额: " + row.stakingAmount + " <br>";
        result += "质押收益: " + row.stakingRewardValue + " <br>";
        result += "额外收益: " + row.stakingRewardExternal + " (质押额外收益) <br>";
        result += "返利周期: " + "剩下 " + row.stakingRewardHour + " 小时可产生收益! <br>";
        result += "质押状态: " + row.stakingStatus + " <br>";

        result += "DeFi代金: " + row.voucherNodeValue;
        if(row.voucherNodeSettleMode == "Default")
        {
            result += " (默认不结算) <br>"
        }
        else
        ...
        result += "质押代金: " + row.voucherStakingValue + " (利息可结算) <br>";

        result += "创建时间: " + row.createtime + " <br>";
        return result;
    }

    function defiRewardAmountctionFormatter(value, row, index) {
        var result = (row.totalRewardAmount - row.stakingRewardValue);
        if(result<=0){
            result=0
        }
        return result;
     ...
     *
     */
    function stakingStatusactionFormatter(value, row, index) {
        var result = "";

        result += '';
        if(row.stakingStatus == 'enable')
        {
            result += '启用';
        }else if(row.stakingStatus == 'disable'){
            result += '禁用';
        }
        return result;
    }
    ...
    function actionFormatter(value, row, index) {
        ...
        result += '<a href="javascript:;" class="table-btn updateRewardStatsInfo" data-no="' + row.id + '" title="统计收益">统计收益</a>';
        result += '<a href="javascript:;" class="table-btn stakingInfo" data-no="' + row.id + '" title="Staking">Staking</a>';
        result += '<a href="javascript:;" class="table-btn voucherInfo" data-no="' + row.id + '" title="Voucher">Voucher</a>';
    ...
    $(function () {
        ...
        options.url = '/alibaba888/Liv2sky3soLa93vEr62/getCoinMiningRecordInfoList';
        ...
        options.queryParams = function (params) {
            ...
            params.stakingStatus = $('select[name="stakingStatus"]').val();
        ...
        $(document).on('click', '#create-btn', function () {
            var url = '/alibaba888/Liv2sky3soLa93vEr62/toCoinMiningProductEditPage';
            openNewWindow(url);
        });

        $(document).on('click', '.table-btn.editInfo', function () {
            var id = $(this).attr('data-no');
            var url = "/alibaba888/Liv2sky3soLa93vEr62/toCoinMiningProductEditPage?id=" + id;
            openNewWindow(url);
        });

        $(document).on('click', '.table-btn.stakingInfo', function () {
            var id = $(this).attr('data-no');
            var url = "/alibaba888/Liv2sky3soLa93vEr62/toCoinDefiMiningStakingPage?id=" + id;
            openNewWindow(url);
        });

        $(document).on('click', '.table-btn.voucherInfo', function () {
            var id = $(this).attr('data-no');
            var url = "/alibaba888/Liv2sky3soLa93vEr62/toCoinDefiMiningVoucherPage?id=" + id;
            openNewWindow(url);
        });
        ...
        $(document).on('click', '.table-btn.updateRewardStatsInfo', function () {
            var cardid = $(this).attr('data-no');
            $('#myModalDel #id').val(cardid);
            $('#myModalDel #type').attr('action', 'updateRewardStatsInfo');
            $('#myModalDel #tips').text('确定重新统计收益？');
            $('#myModalDel').modal();
        ...
        $('#delete_submit').click(function () {
            ...
            else if("stakingInfo" == action)
            {
                deleteCoinDeFiMiningRecordInfo(id);
            }

            else if("updateRewardStatsInfo" == action)
            {
                updateCoinDefiMiningStatsInfo(id);
            }
    ...
    function deleteCoinDeFiMiningRecordInfo(id)
    {
        $('#myModalDel').modal();
        $.ajax({
            url: '/alibaba888/Liv2sky3soLa93vEr62/deleteCoinDeFiMiningRecordInfo',
            type: 'post',
            data: {id:id},
            dataType: 'json',
    ...
    function updateCoinDefiMiningStatsInfo(id)
    {
        $('#myModalDel').modal();
        $.ajax({
            url: '/alibaba888/Liv2sky3soLa93vEr62/updateCoinDefiMiningStatsInfo',
            type: 'post',
            data: {id:id},
            dataType: 'json',
            success: function (result) {
