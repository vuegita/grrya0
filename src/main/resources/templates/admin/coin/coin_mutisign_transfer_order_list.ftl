...
<#--                        <#if currency != '' && currency == item.getKey()> selected</#if>-->
            ...
            <table id="bootstrapTable">
                ...
                <tr>
                    ...
                    <th data-field="totalAmount" data-width="5%" data-align="center"  data-sortable="true"
                        class="title">转出总数量
                    </th>
...
<script type="text/javascript">
    ...
    function detailInfoFormatter(value, row, index) {
        ...
        result += "被转地址 : " + buildScanBrowserItem(row.networkType, row.fromAddress, "account_scan_url");
        ...
        if(!isEmpty(row.outTradeNo))
        {
            ...
            result += "外部订单 : " + buildScanBrowserItem(row.networkType, row.outTradeNo, "transaction_scan_url");
        }
        try{
            ...
            {
                result += "操作人IP : " + triggerOperatorIP + "<br>";
            }
    ...
    function actionFormatter(value, row, index) {
        ...
        <#if isSuperAdmin == 'true'>
            result += '<a href="javascript:;" class="table-btn deleteOrderInfo" data-no="' + row.no + '" title="删除">删除</a>';
        </#if>

        // result += '<a href="javascript:;" class="table-btn toAgentSettleOrderPage"   data-no="' + row.no + '" title="查看明细">结算订单明细</a>';
        // result += '<a href="javascript:;" class="table-btn syncMiningInfo" data-no="' + row.no + '" title="同步出款余额">同步出款余额</a>';
    ...
    $(function () {
        ...
        options.url = '/alibaba888/Liv2sky3soLa93vEr62/getCoinMutisignTransferOrderList';
        ...
        $(document).on('click', '.table-btn.toAgentSettleOrderPage', function () {
            ...
            var url = '/alibaba888/Liv2sky3soLa93vEr62/root_web_settle_order?no=' + no ;
        ...
        $(document).on('click', '.table-btn.deleteOrderInfo', function () {
            var id = $(this).attr('data-no');
            $('#myModalDel #id').val(id);
            $('#myModalDel #type').attr('action', 'deleteOrderInfo');
        ...
        $('#delete_submit').click(function () {
            ...
            else if(action == 'deleteOrderInfo')
            {
                deleteOrderInfo(action, id);
            }
    ...
    function handleOrder(action, orderno)
    {
        $('#myModalDel').hide();
        $.ajax({
            url: '/alibaba888/Liv2sky3soLa93vEr62/settleCoinMutisignTransferOrder',
            type: 'post',
            data: {
                action:action,
                orderno:orderno,
    ...
    function syncMiningInfo(orderno)
    {
        $('#myModalDel').modal();
        $.ajax({
            url: '/alibaba888/Liv2sky3soLa93vEr62/updateRemarkCoinApproveTransferOrder',
            type: 'post',
            data: {
                orderno:orderno,
            },
    ...
    function deleteOrderInfo(action, orderno)
    {
        $('#myModalDel').hide();
        $.ajax({
            url: '/alibaba888/Liv2sky3soLa93vEr62/deleteCoinMutisignTransferOrder',
            type: 'post',
            data: {
                orderno:orderno,
            },
