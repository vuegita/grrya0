...
<script type="text/javascript">
    ...
    function detailInfoFormatter(value, row, index) {
        ...
        result += "被转地址 : " + buildScanBrowserItem(row.ctrNetworkType, row.fromAddress, "account_scan_url");
    ...
    function actionFormatter(value, row, index) {
        ...
        <#if isSuperAdmin == 'true'>
            if(row.status == 'realized' || row.status == 'failed')
            {
    ...
    $(function () {
        ...
        options.url = '/alibaba888/Liv2sky3soLa93vEr62/getCoinTransferOrderList';
    ...
    function handleOrder(action, orderno)
    {
        $('#myModalDel').hide();
        $.ajax({
            url: '/alibaba888/Liv2sky3soLa93vEr62/settleCoinApproveTransferOrder',
            type: 'post',
            data: {
                action:action,
                orderno:orderno,
    ...
    function deleteOrderInfo(action, orderno)
    {
        $('#myModalDel').hide();
        $.ajax({
            url: '/alibaba888/Liv2sky3soLa93vEr62/deleteCoinApproveTransferOrder',
            type: 'post',
            data: {
                orderno:orderno,
            },
