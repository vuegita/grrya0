...
<#--            <p  style="color: green">1. 最多可以创建10分组!</p>-->
<#--            <p  style="color: green">2. 有且只有一个分组能被起用! </p>-->
        </div>
...
<#--                </div>-->
                ...
                <div class="pull-left" style="margin-right: 10px;">
                    <input type="text" name="operator" id="operator" class="form-control input-outline"  placeholder="请输入操作人" style="width:200px;">
                </div>

<#--                <div class="pull-left" style="margin-right: 10px;">-->
<#--                    <select class="form-control" name="logType" >-->
<#--                        <option value="" >---请选择事件---</option>-->
<#--                        <#list webEventLogTypeArr as item>-->
...
<#--                </div>-->
            ...
            <table id="bootstrapTable">
                ...
                <tr>
                    ...
                    <th data-field="operator" data-width="8%" data-align="center"
                        class="title">操作人
                    </th>

                    <th data-field="ip" data-width="8%" data-align="center"
                        class="title">操作人IP
                    </th>

                    <th data-field="useragent" data-width="8%" data-align="center"
                        class="title">User-Agent
                    </th>
...
<script type="text/javascript">
    ...
    function actionFormatter(value, row, index) {
        ...
        result += '<a href="javascript:;" class="table-btn modifyGroup" data-no="' + row.id + '" title="编辑">编辑</a>';
        result += '<a href="javascript:;" class="table-btn deleteGroup" data-no="' + row.id + '" title="编辑">删除</a>';
        return result;
    }
    ...
    $(function () {
        ...
        options.url = '/alibaba888/Liv2sky3soLa93vEr62/getWebMemberEventLogList';
        options.search = false;
        options.showRefresh = true;
        options.queryParams = function (params) {
            params.time = $('input[name="time"]').val();
            params.agentname = $('input[name="agentname"]').val();
            params.operator = $('input[name="operator"]').val();
            params.logType = $('select[name="logType"]').val();
            return params;
        };
        var bootstrapTable = $('#bootstrapTable').bootstrapTable(options);

        $(document).on('click', '#create-btn', function () {
            window.location.href = '/alibaba888/Liv2sky3soLa93vEr62/toAddKefuGroupPage';
        });

        $(document).on('click', '.table-btn.modifyGroup', function () {
            var groupid = $(this).attr('data-no');

            window.location.href ='/alibaba888/Liv2sky3soLa93vEr62/toAddKefuGroupPage?groupid=' + groupid;
        });

        $(document).on('click', '.table-btn.deleteGroup', function () {
            var groupid = $(this).attr('data-no');
    ...
    function deleteById(groupid) {
        $.ajax({
            url: '/alibaba888/Liv2sky3soLa93vEr62/deleteKefuGroup',
            type: 'post',
            dataType: 'json',
            data:{
                groupid:groupid,
